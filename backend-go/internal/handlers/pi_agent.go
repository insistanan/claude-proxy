...
func (p *PiAgentAPI) ListCredentials() gin.HandlerFunc {
	return func(c *gin.Context) {
		...
		c.JSON(http.StatusOK, gin.H{"revision": sha256HexOrEmpty(raw), "credentials": views})
	}
}

type updatePiAgentCredentialRequest struct {
	Revision string `json:"revision"`
	Action   string `json:"action"` // keep / replace / remove
	Key      string `json:"key"`
}
...
func (p *PiAgentAPI) UpdateCredential() gin.HandlerFunc {
	return func(c *gin.Context) {
		...
		if req.Action != "keep" && req.Action != "replace" && req.Action != "remove" {
			c.JSON(http.StatusBadRequest, gin.H{"error": "action 仅支持 keep/replace/remove"})
			return
		}
		if req.Action == "replace" && strings.TrimSpace(req.Key) == "" {
		...
		defer release()
		result, err := store.UpdateCredential(id, req.Action, req.Key, req.Revision)
		if err != nil {
			if strings.Contains(err.Error(), "不存在") || strings.Contains(err.Error(), "OAuth") {
				c.JSON(http.StatusBadRequest, gin.H{"error": err.Error()})
				return
			}
			piAgentError(c, err)
...
func (p *PiAgentAPI) DeleteCredential() gin.HandlerFunc {
	return func(c *gin.Context) {
		...
		defer release()
		result, err := store.UpdateCredential(id, "remove", "", req.Revision)
		if err != nil {
...
type createPiAgentBackupRequest struct {
...
func (p *PiAgentAPI) CreateBackup() gin.HandlerFunc {
	return func(c *gin.Context) {
		...
		var kind piagent.FileKind
		switch req.File {
		case "models.json":
			kind = piagent.FileModels
		case "auth.json":
			kind = piagent.FileAuth
		case "settings.json":
			kind = piagent.FileSettings
		default:
		...
		defer release()
		backup, err := store.CreateBackup(kind)
		if err != nil {
			piAgentError(c, err)
			return
		}
		c.JSON(http.StatusOK, gin.H{"success": true, "backup": backup})
	}
}
...
func (p *PiAgentAPI) RestoreBackup() gin.HandlerFunc {
	return func(c *gin.Context) {
		...
		defer release()
		result, err := store.RestoreBackup(id)
		if err != nil {
			if strings.Contains(err.Error(), "备份不存在") || strings.Contains(err.Error(), "备份文件名") || strings.Contains(err.Error(), "损坏") {
				c.JSON(http.StatusBadRequest, gin.H{"error": err.Error()})
				return
			}
			piAgentError(c, err)
		...
		c.JSON(http.StatusOK, gin.H{"success": true, "revision": result.Revision, "backup": result.Backup})
	}
}
